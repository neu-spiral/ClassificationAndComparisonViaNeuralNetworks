...
nNegComp = 594
################################
file_names = ['_abs_auc.txt', '_abs_acc.txt', '_abs_f1.txt', '_comp_auc.txt', '_comp_acc.txt', '_comp_f1.txt']
eval_models = []
# fetch the best model for each metric
for file_name in file_names:
    with open(dir + '3_best' + file_name, 'r+') as f:
        content = f.readlines()
    ...
    f.close()
    with open(dir + '0' + file_name, 'r+') as f:
        content = f.readlines()
    # get each line as a list
    content = [x.strip() for x in content]
    # remove white spaces
