...
for file_name in file_names:
    ...
    for alpha in ['Alpha: 0.0', 'Alpha: 0.25', 'Alpha: 0.5', 'Alpha: 0.75', 'Alpha: 1.0']:
        ...
        values_str = np.array([content[line_ind + 1] for line_ind in range(len(content)) if (alpha in content[line_ind])])
        values = np.array([float(elm) if '&' not in elm else 0 for elm in values_str])  # check if really is a float
        max_ind = np.argmax(values)
        best_params.append(params[max_ind] + ' & ' + abs_losses[best_abs_loss_ind] + ' & ' + comp_losses[best_comp_loss_ind])
